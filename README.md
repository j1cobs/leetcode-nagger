...
A Discord bot that reads your DSA study tracker from Notion and nags you
daily, via GitHub Actions, until you actually do the work. Not a
...
posts to Discord if so, staying silent otherwise. It's deliberately boring
infrastructure, so the interesting part (spaced repetition and honest
...
mistakes to the topic/pattern that causes them, say "mixing recursion
paradigms" tagged to Backtracking, the bot checks each new problem's
`Pattern` against it and adds a one-line warning before you start. It
catches the mistake before it happens instead of diagnosing it after.
...
- `.github/workflows/nag.yml`: the daily cron (fires within a 5pm-11pm
  ...
  same-day dedup guard so a delayed run can't double-post), plus a
...
- `state.json`: `last_congratulated_week_start`, `last_week_seen`,
  `streak`, `last_run_date`, and `schedule_offset_days`. Committed back by
  the workflow after any run that changes it.
- `requirements.txt`: `requests`, `python-dotenv`, and `tzdata` (Windows
  only, via an environment marker, since Windows has no system IANA
  timezone database for `zoneinfo` to read).
...
NEETCODE150_DATABASE_ID=       # your Tracker database ID
