...
- Local runs need a `.env` (copy from `.env.example`): `NOTION_TOKEN`, `BLIND75_DATABASE_ID`, `DISCORD_WEBHOOK_URL` are required; `SCHEDULE_DATABASE_ID`, `WEAK_PATTERNS_DATABASE_ID`, `DISCORD_USER_ID` are optional.
...
- Required GitHub Actions secrets mirror the `.env` vars: `NOTION_TOKEN`, `BLIND75_DATABASE_ID`, `DISCORD_WEBHOOK_URL` required; `SCHEDULE_DATABASE_ID`, `WEAK_PATTERNS_DATABASE_ID`, `DISCORD_USER_ID` optional.
